...
func (s *SampleBuilder) Push(p *rtp.Packet, gapSeq chan uint16) {
	log.Infof("sn=%d", p.SequenceNumber)
...
func (s *SampleBuilder) buildSample(firstBuffer uint16) *media.Sample {
	log.Infof("buildSample firstBuffer=%d", firstBuffer)
	data := []byte{}
	...
	for i := firstBuffer; s.buffer[i] != nil; i++ {
		if s.buffer[i].Timestamp != s.buffer[firstBuffer].Timestamp {
			...
			log.Infof("buildSample return firstBuffer=%v, samples=%v, lastTimeStamp=%v", firstBuffer, samples, lastTimeStamp)
